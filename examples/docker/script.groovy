...
start {
    plan {
        arguments {
            argument(name: 'var_host', value: "${host_ip}")
            argument(name: 'var_influxdb_host', value: "${influx_ip}")
        }
        ...
        backend(name: 'InfluxDb Backend', enabled: true) {
            arguments {
                ...
                argument(name: 'influxdbUrl', value: 'http://${var_influxdb_host}:8086/write?db=jmeter')
